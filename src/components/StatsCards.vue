...
const connections = computed(() => {
  const d = data.value;
  // Fallback to finding the peer with the same ID as the local node
  return (d?.peers?.find(p => p.id === d.peer_id)?.connections) || [];
});

const totalBandwidth = computed(() =>
  connections.value.reduce((sum: number, c: Connection) => sum + (c.bandwidth_mbps || 0), 0) || 0
);

const avgLatency = computed(() => {
  const validConnections = connections.value.filter((c: Connection) => c.latency_ms || (c.latency_history && c.latency_history.length > 0));
  if (validConnections.length === 0) return 0;

  return validConnections.reduce((sum: number, c: Connection) => {
...
///|
const routesCount = computed(() => data.value?.routes?.length ?? 0);
</script>
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div class="stat bg-base-100">
      ...
      <div class="stat-title text-base-content/60">Known Routes</div>
      <div class="stat-value text-success text-3xl">{{ routesCount }}</div>
      <div class="stat-desc">Routing table entries</div>
    </div>
  </div>
</template>
